...
class ComponentPrice(Base):
    ...
    create_on = Column(DateTime,
                       default=datetime.utcnow)  # Column(Integer, nullable=False, default=timestamp.current_time_stamp())
    update_by = Column(String, default=None)
    update_on = Column(DateTime, default=datetime.utcnow)

    def __repr__(self):
        return '<Component id: {}, Component name: {}>'.format(self.component_id, self.component_product_name)
...
class CostAttributes(Base):
    ...
    def __repr__(self):
        return '<Attribute id: {}, Attribute text: {}>'.format(self.attribute_id, self.attribute_text)

    @property
    def serializable_name(self):
        return self.attribute_shrot_text
